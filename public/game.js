(() => {
  ...
  async function copyGameSearchName() {
    const status = document.getElementById('copyGameNameStatus');

    try {
      if (navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(GAME_SEARCH_NAME);
      } else {
        const input = document.createElement('textarea');
        input.value = GAME_SEARCH_NAME;
  ...
  function showGate(continueOnSite = false) {
    ...
    if (continueOnSite || playerState.registered) {
      ...
      gateText.innerHTML = 'You are registered, you can continue the game on the website <br>Search by name: <b1>Doodle Crash</b1></br>';
  ...
  gateText.addEventListener('click', event => {
    if (event.target?.id === 'copyGameName') {
